...
### Debug Logging
Horizontal-scroll diagnostics are disabled by default so live testing can focus on the shared `[MBC Trace]` output. The logger prefixes every message with `[MBC HorizontalScroll Debug]`, uses warning-level console output for visibility, and includes abort reasons plus pin-spacer measurements during refresh. Set `window.MBC_HORIZONTAL_SCROLL_DEBUG = true` before the runtime loads to enable these logs.
