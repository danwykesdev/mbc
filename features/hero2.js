...
(function () {
  ...
  function initHero2(scope) {
    var root = scope.querySelector('.hero-animate');
    if (!root || typeof gsap === 'undefined') return null;

    if (window.__homeHero2MM) {
      try { window.__homeHero2MM.revert(); } catch (_) {}
      window.__homeHero2MM = null;
    }

    if (window.__homeHero2TL) {
      try { window.__homeHero2TL.kill(); } catch (_) {}
      window.__homeHero2TL = null;
    }
